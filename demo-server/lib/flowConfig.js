...
function resolve(flowId, stepId, body = {}) {
  ...
  switch (key) {
    ...
    case 'flow1.smoke':
      return { cmd: 'bash', args: ['scripts/smoke-test.sh', 'flow1'],
    ...
    case 'flow2.smoke':
      return { cmd: 'bash', args: ['scripts/smoke-test.sh', 'flow2'],
               env: { SMARTRETAIL_ENV: ENV } };
    ...
    case 'flow3.smoke':
      return { cmd: 'bash', args: ['scripts/smoke-test.sh', 'flow3'],
               env: { SMARTRETAIL_ENV: ENV } };

    // ── Flow 4 ─────────────────────────────────────────────────────────────
    case 'flow4.smoke':
      return { cmd: 'bash', args: ['scripts/smoke-test.sh', 'flow4'],
               env: { SMARTRETAIL_ENV: ENV } };

    // ── Flow 8 ─────────────────────────────────────────────────────────────
    case 'flow8.smoke':
      return { cmd: 'bash', args: ['scripts/smoke-test.sh', 'flow8'],
               env: { SMARTRETAIL_ENV: ENV } };

    // ── Flow 9 ─────────────────────────────────────────────────────────────
    case 'flow9.smoke':
      return { cmd: 'bash', args: ['scripts/smoke-test.sh', 'flow9'],
               env: { SMARTRETAIL_ENV: ENV } };
