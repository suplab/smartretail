...
description: API Contract Guardian. Use to check whether a proposed API change is breaking or non-breaking, enforce OpenAPI versioning policy, validate that all endpoints declare required error responses, verify X-Idempotency-Key headers on mutation endpoints, and check whether the TypeScript client needs regeneration. Trigger before committing any {service}-api.yaml change. Read-only.
...
4. Generated files must NOT be committed to git (they're in `.gitignore`)
