...
name: db-admin
description: >
Use for Flyway migration authoring, schema design review, PostgreSQL index strategy,
query optimisation, cross-schema boundary enforcement, and EXPLAIN ANALYZE
interpretation. Trigger when creating new migrations, designing tables, debugging slow
queries, or reviewing anything that touches .sql files or db/migration/.
...
- [ ] Named parameters ( :param ) via NamedParameterJdbcTemplate — no string concat
...
2. `docs/LOCAL_DEV.md` - Running Migrations
   1. .claude/memory/schema-evolution.md   — migration history and rationale
3. Run  mvn flyway:info -pl backend/migrations   to see current applied state
