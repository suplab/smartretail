...
In local mode LocalStack provides both Kinesis and DynamoDB. The `scripts/localstack-init.sh` script creates the stream, the DLQ, and the Lambda function pointing at the built JAR. SIS runs on `localhost:8080`.
