...
# cdk-demo-full target: SIS, IMS, RE, ARS, DFS, SUP (all 6 services).
#
# Usage:
#   ./environments/demo-full/scripts/deploy-services-demo-full.sh [OPTIONS]
#
# Options:
#   --env       <demo-full|...>     Environment name (default: $SMARTRETAIL_ENV or demo-full)
#   --profile   <aws-profile>       AWS CLI profile (default: smartretail-demo-full)
...
#   --deploy-cdk                    Also redeploy Full-ComputeStack via CDK to update the ECS
#                                   task definition (required when env vars / profile changes)
#   --wait                          Wait for every ECS service to reach steady state
#
# Examples:
#   ./environments/demo-full/scripts/deploy-services-demo-full.sh --env demo-full --deploy-cdk --wait
#   ./environments/demo-full/scripts/deploy-services-demo-full.sh --env demo-full --services re,ars --skip-build

set -euo pipefail

# ── Defaults ──────────────────────────────────────────────────────────────────
ENV="${SMARTRETAIL_ENV:-demo-full}"
...
echo "=================================================="
echo " SmartRetail — Deploy Services (demo-full)"
echo " env:      ${ENV}"
echo " cluster:  ${CLUSTER}"
echo " services: ${SERVICES}"
echo "=================================================="

# ── 0. CDK: update Full-ComputeStack task definition (optional) ───────────────
if [[ "$DEPLOY_CDK" == true ]]; then
  echo ""
  echo "▶  Deploying Full-ComputeStack via CDK (updates ECS task definitions)..."
  CDK_DIR="${REPO_ROOT}/environments/demo-full/infra"
  (cd "$CDK_DIR" && \
    SMARTRETAIL_ENV="$ENV" npx cdk deploy Full-ComputeStack \
      --require-approval never \
      --profile "$PROFILE" \
      --no-rollback)
  echo "   ✅ Full-ComputeStack deployed"
fi
