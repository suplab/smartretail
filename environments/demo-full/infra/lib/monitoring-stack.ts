...
export class MonitoringStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: MonitoringStackProps) {
    ...
    cdk.Tags.of(this).add("Name", "smartretail-monitoring-full");
    ...
    for (const svc of SERVICES) {
      new logs.MetricFilter(this, `${svc}ErrorFilter`, {
        logGroup: logs.LogGroup.fromLogGroupName(this, `${svc}LgRef`, `/smartretail/${svc}/${srEnv}`),
    ...
    const apigw5xxAlarm = alarm("ApiGw5xxAlarm", "APIGW-5xxErrors", apigwM("5XXError", "Sum", p5), 10, 1);

    const apigwLatencyAlarm = alarm("ApiGwLatencyAlarm", "APIGW-p99Latency", apigwM("Latency", "p99", p5), 3000, 2);

    const rdsCpuAlarm = alarm("RdsCpuAlarm", "RDS-CPUHigh", rdsM("CPUUtilization", "Average", p10), 80, 2);

    const firehoseFreshnessAlarm = alarm(
      "FirehoseFreshnessAlarm", "Firehose-DeliveryFailed",
    ...
    dashboard.addWidgets(
      new cloudwatch.TextWidget({
        markdown: [
          ...
          `**Services:** SIS · IMS · RE · ARS · DFS · SUP  |  **Env:** ${srEnv}  |  **Stack:** cdk-demo-full`,
        ].join("\n"),
        width: 24,
        height: 2,
      }),
