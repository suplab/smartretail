...
docker-build-sis:
	docker buildx build --platform linux/arm64 --load -t smartretail-sis:local backend/services/sis/

docker-build-all:
	for svc in sis ims re ars dfs sup; do \
	    docker buildx build --platform linux/arm64 --load -t smartretail-$$svc:local backend/services/$$svc/; \
	done

docker-build-batch-post-processor:
	docker buildx build --platform linux/arm64 --load -t smartretail-batch-post-processor:local backend/adapters/batch-post-processor/

docker-build-ml-trigger:
	docker buildx build --platform linux/arm64 --load -t smartretail-ml-trigger:local backend/adapters/ml-trigger/

docker-build-lambda: docker-build-batch-post-processor docker-build-ml-trigger

docker-build-flyway-arm64:
	docker buildx build --platform linux/arm64 --pull --load -t smartretail-flyway:local backend/migrations/

docker-build-flyway-amd64:
	docker buildx build --platform linux/amd64 --pull --load -t smartretail-flyway:local backend/migrations/
